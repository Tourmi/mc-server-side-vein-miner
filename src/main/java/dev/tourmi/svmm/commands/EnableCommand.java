...
public class EnableCommand {
    public static LiteralArgumentBuilder<CommandSourceStack> getCommand() {
        return net.minecraft.commands.Commands.literal("enable")
                .executes(EnableCommand::executes);
    }
    ...
    public static int executes(CommandContext<CommandSourceStack> commandContext) {
        if (!(commandContext.getSource().getEntity() instanceof Player)) {
            Commands.sendMessage(commandContext, "The enable command needs to be run by a player.");
            return 0;
        }

        ClientConfig cfg = ClientConfigs.getClientConfig(commandContext.getSource().getPlayer().getUUID());
        cfg.MOD_DISABLED.set(false);
        cfg.SPEC.save();

        Commands.sendMessage(commandContext, "Vein Miner is now enabled");
